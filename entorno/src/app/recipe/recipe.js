...
// Crear Lista de Ingredientes con cols dentro de los row
// h7, e imagen

let numIngredientRows = ingredientsList.length/4;
...
function genRow(ingredientsP, measuresP){
    ...
    let ingLength = ingredientsP.length;
    row.className = "row";
    for (let i = 0; i<ingLength; i++){
        row.appendChild(genIngredient(measuresP[i] +" " +ingredientsP[i], 'https://www.themealdb.com/images/ingredients/'+ingredientsP[i]+'.png'))
    }
...
console.log(dividedIngredients)
console.log(dividedMeasures)
console.log(dividedIngredients.length)

if((ingredientsList.length%4 )!=0){
    numIngredientRows = numIngredientRows +1;
}

for (let i=0; i<dividedIngredients.length;i++){
    colsList.appendChild(genRow(dividedIngredients[i], dividedMeasures[i]));

   // console.log(dividedIngredients[i])
}
